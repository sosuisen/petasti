...
const createInstaller = async () => {
  ...
  while (true) {
    ...
    await packager({
      ...
      console.log(`Error in Packager: ${e.message}`);
      const [txt, capture] = e.message.match(/Failed to locate module "(.+?)"/);
      if (capture) {
        console.log(`Install ${capture}`);
        const result = execSync(`pnpm i ${capture}`);
