 ...
 */
import { Geometry, Geometry2D, GeometryXY, ICard, RelativePositionOfCardUrl } from '../modules_common/types';
...
export const calcRelativePositionOfCardUrl = (targetCardUrl: string): RelativePositionOfCardUrl => {
  ...
  for (const card of cacheOfCard.values()) {
    ...
    const targetVector = { x: currentCentroid.x - targetCentroid.x, y: currentCentroid.y - targetCentroid.y };
    const targetLength = Math.sqrt(Math.pow(targetVector.x, 2) + Math.pow(targetVector.y, 2));
    if (targetLength === 0) {
